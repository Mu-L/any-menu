...
export function initApi() {
  ...
  global_setting.api.getCursorXY = async () => {
    const ret: any = await invoke("get_caret");
    if (typeof ret === 'string' || ret.length < 4) { // 失败、错误信息
    ...
    const x = ret[0], y = ret[1], selectedText = ret[2], appName = ret[3]
    global_setting.state.selectedText = selectedText && selectedText.length > 0 ? selectedText : undefined
  ...
  */
  ...
  global_setting.other.app_show = getMsg_and_showWindow
